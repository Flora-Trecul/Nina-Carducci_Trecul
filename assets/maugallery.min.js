...
element.wrap(`<div class='item-column mb-4${columnClasses}'></div>`)}else{console.error(`Columns should be defined as numbers or objects. ${typeof columns} is not supported.`)}},moveItemInRowWrapper(element){element.appendTo(".gallery-items-row")},responsiveImageItem(element){if(element.prop("tagName")==="IMG"){element.addClass("img-fluid")}},openLightBox(element,lightboxId){const jpgSrc=element.find("img").attr("src")
const imgAlt="Image affichée dans la modale au clic : "+element.find("img").attr("alt")
$(`#${lightboxId}`).find(".lightboxImage").attr("src",jpgSrc).attr("alt",imgAlt)
$(`#${lightboxId}`).modal("toggle")},prevImage(){let activeImage=null;$("picture.gallery-item").each(function(){if($(this).find("img").attr("src")===$(".lightboxImage").attr("src")){activeImage=$(this).children("img")}});let activeTag=$(".tags-bar span.active-tag").data("images-toggle");let imagesCollection=[];if(activeTag==="all"){$(".item-column").each(function(){if($(this).children("picture").length){imagesCollection.push($(this).children("picture").find("img"))}})}else{$(".item-column").each(function(){if($(this).children("picture").data("gallery-tag")===activeTag){imagesCollection.push($(this).children("picture").find("img"))}})}
let index=0,next=null;$(imagesCollection).each(function(i){if($(activeImage).attr("src")===$(this).attr("src")){index=i}});next=imagesCollection[index-1]||imagesCollection[imagesCollection.length-1];$(".lightboxImage").attr("src",$(next).attr("src"));$(".lightboxImage").attr("alt","Image affichée dans la modale au clic : "+$(next).attr("alt"))},nextImage(){let activeImage=null;$("picture.gallery-item").each(function(){if($(this).find("img").attr("src")===$(".lightboxImage").attr("src")){activeImage=$(this).children("img")}});let activeTag=$(".tags-bar span.active-tag").data("images-toggle");let imagesCollection=[];if(activeTag==="all"){$(".item-column").each(function(){if($(this).children("picture").length){imagesCollection.push($(this).children("picture").find("img"))}})}else{$(".item-column").each(function(){if($(this).children("picture").data("gallery-tag")===activeTag){imagesCollection.push($(this).children("picture").find("img"))}})}
let index=0,next=null;$(imagesCollection).each(function(i){if($(activeImage).attr("src")===$(this).attr("src")){index=i}});next=imagesCollection[index+1]||imagesCollection[0];$(".lightboxImage").attr("src",$(next).attr("src"));$(".lightboxImage").attr("alt","Image affichée dans la modale au clic : "+$(next).attr("alt"))},createLightBox(gallery,lightboxId,navigation){gallery.append(`<div class="modal fade" id="${
